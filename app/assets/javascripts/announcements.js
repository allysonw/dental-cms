...
class Announcement {
  constructor(attributes) {
    ...
    this.createdAt = moment(attributes.created_at).format('MMM DD, YYYY - h:mm A');
  }
  ...
  formatAnnouncement() {
    ...
    formattedAnnouncement += "<i>Posted @ " + this.createdAt + "<i>";
...
function bindAnnouncementClickHandlers() {
  $("#announcement-next-link").on("click", function(e) {
    ...
    getNextAnnouncement(announcement_id)
  });
}

// GETs the next announcemnet, provided with the
// current announcement's id
function getNextAnnouncement(announcement_id) {
  $.ajax({
      method: "GET",
  ...
  .done(function (json) {
    // json is an Array of announcements
